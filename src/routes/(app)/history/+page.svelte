  ...
  const filteredEntries = $derived.by(() => {
    ...
    return normalizedEntries
      ...
      .filter((entry) => {
        ...
        const haystack = `${entry.title} ${entry.overview ?? ''} ${entry.genres.join(' ')}`.toLowerCase();
    ...
    {:else if normalizedEntries.length === 0}
      <Card class="border-border bg-background/60 backdrop-blur">
        <CardContent class="space-y-4 p-6 text-center">
          ...
          <Button type="button" variant="secondary" class="mx-auto w-fit" href="/(app)/explore/movies">
            Browse titles
          </Button>
        </CardContent>
      </Card>
    ...
    {:else if filteredEntries.length === 0}
      <Card class="border-border bg-background/60 backdrop-blur">
        <CardContent class="space-y-4 p-6 text-center">
          ...
          <Button type="button" variant="outline" class="mx-auto w-fit gap-2" onclick={resetFilters}>
            <RefreshCw class="size-4" />
            Clear filters
          </Button>
        </CardContent>
            ...
            <CardContent class="flex flex-col gap-4 p-4 sm:flex-row sm:items-start sm:gap-6 sm:p-6">
              <div class="relative h-48 w-full overflow-hidden rounded-lg bg-muted sm:w-32">
                {#if entry.posterPath}
                  ...
                  />
                {:else}
                  <div class="flex h-full w-full items-center justify-center text-xs uppercase tracking-wide text-muted-foreground">
                    No artwork
                  </div>
                {/if}
              </div>
