<script lang="ts">
  ...
  let sortOption = $state<'recent' | 'oldest' | 'alphabetical' | 'rating-desc' | 'release-desc'>('recent');
  ...
  const sortLabels: Record<'recent' | 'oldest' | 'alphabetical' | 'rating-desc' | 'release-desc', string> = {
    recent: 'Newest first',
    oldest: 'Oldest first',
    alphabetical: 'A to Z',
  ...
  const groupedByDate = $derived((() => {
    ...
    for (const movie of sortedMovies) {
      ...
      const normalized = hasValidDate ? new Date(parsed.getFullYear(), parsed.getMonth(), parsed.getDate()) : null;
...
{:else}
  <div class="min-h-screen bg-background text-foreground">
    <main class="container mx-auto space-y-8 px-4 py-8">
      <header class="flex flex-col gap-6 rounded-2xl border border-border bg-card/60 p-6 shadow-sm backdrop-blur md:flex-row md:items-center md:justify-between">
        <div class="space-y-2">
          <div class="flex items-center gap-3 text-muted-foreground">
            <Sparkles class="size-4" />
            <span class="text-sm uppercase tracking-wide">Personal queue</span>
          </div>
      ...
      <section class="flex flex-col gap-4 rounded-2xl border border-border bg-card/40 p-6 shadow-sm backdrop-blur">
        <div class="grid gap-4 lg:grid-cols-12">
          <div class="lg:col-span-4">
            <div class="flex items-center gap-2 rounded-lg border border-border bg-background/70 px-3">
          ...
          <div class="grid gap-4 sm:grid-cols-2 lg:col-span-8 lg:grid-cols-4">
            ...
            <Select type="single" bind:value={selectedMediaType}>
              <SelectTrigger aria-label="Filter by type">
                <span data-slot="select-value">
                  {selectedMediaType === 'movie' ? 'Movies only' : selectedMediaType === 'tv' ? 'TV only' : 'Movies & TV'}
                </span>
              </SelectTrigger>
              <SelectContent>
            ...
            <Select type="single" bind:value={selectedQuality}>
              <SelectTrigger aria-label="Filter by quality">
                <span data-slot="select-value">
                  {selectedQuality === '4k' ? '4K only' : selectedQuality === 'hd' ? 'HD & 4K' : 'Any quality'}
                </span>
              </SelectTrigger>
              <SelectContent>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <Button type="button" variant="secondary" onclick={exportWatchlist}>
              Export list
            </Button>
            <AlertDialog>
              <AlertDialogTrigger
      ...
      {#if sortedMovies.length === 0}
        <section class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card/40 p-16 text-center text-muted-foreground">
          ...
          <p class="mb-6 max-w-md text-sm">
            Use the explore and search tabs to add titles to your watchlist. Filter controls above will help you keep things organised as your queue grows.
          </p>
          <div class="flex flex-wrap items-center justify-center gap-3 text-xs">
            <Badge variant="secondary" class="uppercase tracking-wide">Tip</Badge>
            ...
            <div class="space-y-5">
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-muted-foreground">{group.movies.length} title{group.movies.length === 1 ? '' : 's'} added</p>
                </div>
              ...
              {:else}
                <div class="space-y-3">
                  {#each group.movies as movie (movie.id)}
                    <article class="flex flex-col gap-4 rounded-xl border border-border bg-card/50 p-4 shadow-sm transition hover:border-primary/50 md:flex-row md:items-center">
                      <a
                      ...
                      >
                        {#if movie.posterPath}
                          ...
                          />
                        {:else}
                          <div class="flex h-36 w-24 items-center justify-center rounded-lg bg-muted text-muted-foreground">
                      ...
                      <div class="flex flex-1 flex-col gap-3">
                          ...
                          <div class="space-y-1">
                            ...
                            <div class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <span class="flex items-center gap-1">
                                ...
                                {Number.isFinite(movie.rating) && movie.rating > 0 ? movie.rating.toFixed(1) : 'NR'}
                              </span>
                              <span>
                                {(movie.genres ?? [])
                                  .slice(0, 3)
                        ...
                        <div class="flex flex-wrap items-center gap-2 text-xs">
                          ...
                          {:else if movie.isHD}
                            <Badge variant="secondary" class="bg-blue-500/20 text-blue-200">HD</Badge>
                          {/if}
