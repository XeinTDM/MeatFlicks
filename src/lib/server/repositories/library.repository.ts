...
export const libraryRepository = {
	...
	): Promise<(Collection & { movies: Movie[] }) | null> {
		try {
	...
	async findCollectionMovies(collectionSlug: string, limit?: number): Promise<Movie[]> {
		try {
			const collection = await prisma.collection.findUnique({
				where: { slug: collectionSlug },
				include: { movies: { take: limit } }
			});
	...
	async findGenreMovies(genreName: string, limit?: number): Promise<Movie[]> {
		try {
			return await prisma.movie.findMany({
				where: {
					genres: {
