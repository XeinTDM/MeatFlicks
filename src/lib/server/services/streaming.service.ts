...
const buildCacheKey = (input: ResolveStreamingInput): string => {
  const providerKey = Array.from(new Set(input.preferredProviders ?? [])).sort().join(',');
...
export async function resolveStreaming(input: ResolveStreamingInput): Promise<ResolveStreamingResponse> {
  ...
  streamingResolutionCache.set(cacheKey, response, ttlOverride ? { ttlMs: ttlOverride } : undefined);
