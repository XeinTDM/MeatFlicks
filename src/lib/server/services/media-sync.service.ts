...
export const mediaSyncService = {
     ...
     * Returns immediately.
     */
    async scheduleSync(tmdbId: number, mediaType: 'movie' | 'tv' | 'anime', priority: SyncPriority = 'medium') {
        ...
        syncLimiter.schedule({ priority: priority === 'high' ? 1 : priority === 'medium' ? 5 : 9 }, async () => {
            try {
