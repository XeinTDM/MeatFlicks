...
import type { HomeLibrary, LibraryCollection, LibraryGenre, LibraryMovie } from '$lib/types/library';
...
const buildImdbMap = async (movies: { tmdbId: number | null }[]): Promise<Map<number, string | null>> => {
...
const attachIdentifiers = (movie: LibraryMovie, imdbMap: Map<number, string | null>): LibraryMovie => {
...
export async function fetchHomeLibrary(options: FetchHomeLibraryOptions = {}): Promise<HomeLibrary> {
  ...
  const result = await homeLibraryCache.getOrSet(HOME_LIBRARY_CACHE_KEY, fetchHomeLibraryFromSource);
  return clone(result);
}
