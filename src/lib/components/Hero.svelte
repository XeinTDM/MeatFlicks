<script lang="ts">
	...
	const slides = $derived(
		(() => {
			...
			const seen = $state(new Set<string>());
		...
		{#if isMultiSlide}
			<div class="absolute inset-x-[5%] bottom-8 z-20 flex flex-wrap justify-center gap-2">
				{#each slides as slide, index (slide.key)}
					<button
						type="button"
						class={`group flex max-w-[14rem] items-center gap-2 rounded-full border px-4 py-1 text-xs font-semibold tracking-wide uppercase backdrop-blur transition-colors duration-300 ${index === activeIndex ? 'border-primary/70 bg-background/70 text-primary shadow-sm' : 'border-foreground/20 bg-background/40 text-foreground/80 hover:border-foreground/40 hover:text-foreground'}`}
						onclick={() => goToSlide(index)}
						aria-current={index === activeIndex}
						aria-label={`Show ${slide.movie.title}`}
					>
						<span
							class={`size-2 rounded-full ${index === activeIndex ? 'bg-primary' : 'bg-foreground/40 group-hover:bg-foreground/70'}`}
						></span>
						<span class="truncate">
							{slide.movie.title}
						</span>
