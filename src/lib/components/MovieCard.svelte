...
<a
  href={`/movie/${movie.id}`}
  class="movie-card bg-bg-color-alt hover:shadow-shadow-color group relative h-72 cursor-pointer overflow-hidden rounded-lg transition-transform duration-300 ease-in-out hover:z-10 hover:scale-105 hover:shadow-lg transform-gpu w-48"
  ...
  <div class="relative w-full h-full">
    {#if movie.posterPath}
      <img
        ...
        class="object-cover transition-opacity duration-400 ease-in-out w-full h-full"
      />
    {:else}
    ...
    <div class="absolute top-4 right-4 flex gap-2 opacity-0 transition-all duration-400 ease-in-out group-hover:scale-100 group-hover:opacity-100">
      {#if session?.user}
        <button
          ...
          class={`flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-lg backdrop-blur-sm transition-colors duration-300 ${isInWatchlist ? 'bg-red-600 text-text-color hover:bg-red-700' : 'bg-gray-800/70 text-text-color hover:bg-black/80'}`}
        >
