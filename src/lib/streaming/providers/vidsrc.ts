...
function fallbackSource(context: Parameters<StreamingProvider['fetchSource']>[0], params: URLSearchParams) {
  const embedUrl = `${vidsrc.baseUrl.replace('/api', '')}/embed/${context.mediaType}?${params.toString()}`;
...
async function requestVidsrc(context: Parameters<StreamingProvider['fetchSource']>[0]) {
  ...
  try {
    ...
    return {
      ...
      notes: streamCandidate ? 'Direct stream resolved from VidSrc API.' : 'Embed resolved from VidSrc API.'
    } as const;
  } catch (error) {
