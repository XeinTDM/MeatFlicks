...
const { movies, collections, genres, moviesGenres } = require('./src/lib/server/db/schema');
...
async function checkMovies() {
  try {
    ...
    console.log('Sample movies:', allMovies.map(m => ({ id: m.id, tmdbId: m.tmdbId, imdbId: m.imdbId, title: m.title })));
    ...
    const movie1 = await db.select().from(movies).where(eq(movies.id, '4021522c-b8f7-4d91-a7ec-2cc4a6d712ef')).limit(1);
    console.log('Movie with UUID 4021522c-b8f7-4d91-a7ec-2cc4a6d712ef:', movie1);

    const movie2 = await db.select().from(movies).where(sql`${movies.imdbId} = 'tt0816692'`).limit(1);
