...
CREATE TABLE `movies` (
	...
	`is4K` integer DEFAULT 0 NOT NULL,
	`isHD` integer DEFAULT 0 NOT NULL,
	`language` text,
	`popularity` real,
...
CREATE TABLE `seasons` (
	...
	`created_at` integer NOT NULL,
	`updated_at` integer NOT NULL,
	PRIMARY KEY(`tv_show_id`, `season_number`),
	FOREIGN KEY (`tv_show_id`) REFERENCES `tv_shows`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
...
CREATE INDEX `idx_watchlist_addedAt` ON `watchlist` (`added_at`);